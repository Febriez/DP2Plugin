...
public class ShopManager {
    ...
    private Villager createNPC(Location location, String name) {
    ...
    private MerchantRecipe createTrade(ItemStack ingredient1, ItemStack ingredient2, ItemStack result) {
    ...
    private ItemStack createCurrency(int amount) {
        ItemStack currency = new ItemStack(Material.GOLD_NUGGET, amount);
        ItemMeta meta = currency.getItemMeta();
        meta.setDisplayName("§6화폐");
        meta.setLore(List.of("§7상점에서 사용할 수 있는 화폐입니다."));
        meta.setCustomModelData(2001);
        currency.setItemMeta(meta);
        return currency;
    }
    
    private ItemStack createAbilityRemovalTicket(int amount) {
        ItemStack ticket = new ItemStack(Material.PAPER, amount);
        ItemMeta meta = ticket.getItemMeta();
        meta.setDisplayName("§c능력 삭제권");
        meta.setLore(List.of("§7스킬 삭제 포션과 교환할 수 있습니다."));
