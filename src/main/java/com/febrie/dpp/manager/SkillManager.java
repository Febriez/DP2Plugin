...
public class SkillManager {
        ...
        return playerDataService.getPlayerSkills(player.getUniqueId(), player.getName())
            .thenCompose(skills -> {
                if (hasSkill(skills, skillId)) {
                    player.sendMessage("§c이미 보유하고 있는 스킬입니다!");
                ...
                return playerDataService.updatePlayerSkills(newSkills)
                    .thenApply(v -> {
                        player.sendMessage("§a" + getSkillDisplayName(skillId) + " §a스킬을 습득했습니다!");
    ...
    public boolean removeSkill(Player player, String skillId) {
        return playerDataService.getPlayerSkills(player.getUniqueId(), player.getName())
            .thenCompose(skills -> {
                if (!hasSkill(skills, skillId)) {
                    player.sendMessage("§c보유하지 않은 스킬입니다!");
                    return java.util.concurrent.CompletableFuture.completedFuture(false);
                }
                ...
                return playerDataService.updatePlayerSkills(newSkills)
                    .thenApply(v -> {
                        player.sendMessage("§e" + getSkillDisplayName(skillId) + " §e스킬을 삭제했습니다!");
                        updatePlayerEffects(player, newSkills);
                        return true;
                    });
            }).join();
    ...
    public void updateVirtualLevel(Player player, int newLevel) {
    ...
    public void updatePlayerEffects(Player player, PlayerSkillState skills) {
    ...
    private boolean hasSkill(PlayerSkillState skills, String skillId) {
    ...
    private PlayerSkillState updateSkillState(PlayerSkillState skills, String skillId, boolean value) {
        return switch (skillId) {
    ...
    private List<String> getSkillLore(String skillId) {
        return switch (skillId) {
    ...
    private int getSkillModelData(String skillId) {
        return switch (skillId) {
